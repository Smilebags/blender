                                              ...
                                              int offset)
{
  ...
  switch (type) {
    ...
    case CLOSURE_BSDF_PRINCIPLED_ID: {
      ...
      SceneLinearColor weight = sd->svm_closure_weight * mix_weight;

#  ifdef __SUBSURFACE__
      SceneLinearColor mixed_ss_base_color = subsurface_color * subsurface +
                                          base_color * (1.0f - subsurface);
      SceneLinearColor subsurf_weight = weight * mixed_ss_base_color * diffuse_weight;
      ...
      if (fabsf(average(mixed_ss_base_color)) > CLOSURE_WEIGHT_CUTOFF) {
        if (subsurface <= CLOSURE_WEIGHT_CUTOFF && diffuse_weight > CLOSURE_WEIGHT_CUTOFF) {
          SceneLinearColor diff_weight = weight * base_color * diffuse_weight;
...
#  endif
      ...
      if (diffuse_weight > CLOSURE_WEIGHT_CUTOFF && sheen > CLOSURE_WEIGHT_CUTOFF) {
        ...
        SceneLinearColor m_ctint = m_cdlum > 0.0f ?
                                    base_color / m_cdlum :
                                    one_scene_linear_color();  // normalize lum. to isolate hue+sat

        /* color of the sheen component */
        SceneLinearColor sheen_color = make_scene_linear_color(1.0f - sheen_tint) + m_ctint * sheen_tint;

        SceneLinearColor sheen_weight = weight * sheen * sheen_color * diffuse_weight;
...
#  endif
        if (specular_weight > CLOSURE_WEIGHT_CUTOFF &&
          ...
          SceneLinearColor spec_weight = weight * specular_weight;
          ...
          if (bsdf && extra) {
            ...
            float3 m_ctint = m_cdlum > 0.0f ?
                                 base_color / m_cdlum :
                                        one_scene_linear_color();  // normalize lum. to isolate hue+sat
            SceneLinearColor tmp_col = make_scene_linear_color(1.0f - specular_tint) +
...
#  endif
        if (final_transmission > CLOSURE_WEIGHT_CUTOFF) {
          SceneLinearColor glass_weight = weight * final_transmission;
          SceneLinearColor cspec0 = base_color * specular_tint +
                                 make_scene_linear_color(1.0f - specular_tint);
...
#  endif
        if (clearcoat > CLOSURE_WEIGHT_CUTOFF) {
          ...
          if (bsdf && extra) {
            ...
            bsdf->extra->color = zero_scene_linear_color();
            bsdf->extra->cspec0 = make_scene_linear_color(0.04f);
...
#  endif
    ...
    case CLOSURE_BSDF_MICROFACET_MULTI_GGX_ID: {
      ...
      if (type == CLOSURE_BSDF_REFLECTION_ID)
        ...
        if (bsdf->extra) {
          bsdf->extra->color = stack_load_scene_linear_color(stack, data_node.w);
          bsdf->extra->cspec0 = zero_scene_linear_color();
          bsdf->extra->clearcoat = 0.0f;
    ...
    case CLOSURE_BSDF_MICROFACET_MULTI_GGX_GLASS_ID: {
      ...
      bsdf->extra->color = stack_load_scene_linear_color(stack, data_node.z);
      bsdf->extra->cspec0 = zero_scene_linear_color();
      bsdf->extra->clearcoat = 0.0f;
      ...
      if (bsdf) {
        ...
        switch (parametrization) {
          case NODE_PRINCIPLED_HAIR_DIRECT_ABSORPTION: {
            SceneLinearColor absorption_coefficient = stack_load_scene_linear_color(
          ...
          case NODE_PRINCIPLED_HAIR_PIGMENT_CONCENTRATION: {
            ...
            SceneLinearColor tint_sigma = bsdf_principled_hair_sigma_from_reflectance(
          ...
          case NODE_PRINCIPLED_HAIR_REFLECTANCE: {
            SceneLinearColor color = stack_load_scene_linear_color(stack, color_ofs);
    ...
    case CLOSURE_BSSRDF_RANDOM_WALK_FIXED_RADIUS_ID: {
      ...
      if (bssrdf) {
        ...
        bssrdf->radius = stack_load_scene_linear_color(stack, data_node.z) * param1;
                                                 ...
                                                 uint4 node)
{
  ...
  SceneLinearColor weight = sd->svm_closure_weight;

  if (type == CLOSURE_VOLUME_ABSORPTION_ID) {
    weight = one_scene_linear_color() - weight;
  }
                                                   ...
                                                   int offset)
{
  ...
  if (density > CLOSURE_WEIGHT_CUTOFF) {
    ...
    SceneLinearColor color = sd->svm_closure_weight;
    ...
    SceneLinearColor zero = zero_scene_linear_color();
    SceneLinearColor one = one_scene_linear_color();
    SceneLinearColor absorption_color = max(
        sqrt(stack_load_scene_linear_color(stack, absorption_color_offset)), zero);
    SceneLinearColor absorption = max(one - color, zero) * max(one - absorption_color, zero);
  ...
  if (emission > CLOSURE_WEIGHT_CUTOFF) {
    SceneLinearColor emission_color = stack_load_scene_linear_color(stack, emission_color_offset);
...
ccl_device void svm_node_closure_weight(ccl_private ShaderData *sd,
                                        ...
                                        uint weight_offset)
{
  SceneLinearColor weight = stack_load_scene_linear_color(stack, weight_offset);
                                                  ...
                                                  uint4 node)
{
  ...
  SceneLinearColor weight = stack_load_scene_linear_color(stack, color_offset) * strength;
