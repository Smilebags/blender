 ...
 */
ccl_device_forceinline SceneLinearColor MF_FUNCTION_FULL_NAME(mf_eval)(float3 wi,
                                                                    float3 wo,
                                                                    const bool wo_outside,
                                                                    const SceneLinearColor color,
                                                                    const float alpha_x,
                                                                    const float alpha_y,
                                                                    ccl_private uint *lcg_state,
                                                                    const float eta,
                                                                    bool use_fresnel,
                                                                    const SceneLinearColor cspec0)
{
  ...
  SceneLinearColor throughput = one_scene_linear_color();
  ...
  eval = make_scene_linear_color(val);
#endif
 ...
 */
ccl_device_forceinline SceneLinearColor MF_FUNCTION_FULL_NAME(mf_sample)(float3 wi,
  ...
  for (order = 0; order < 10; order++) {
    ...
    if (use_fresnel) {
      if (!next_outside) {
        ...
        SceneLinearColor t_color = interpolate_fresnel_color(wi_prev, wm, eta, F0, cspec0);
...
#else /* MF_MULTI_GLOSSY */
    if (use_fresnel) {
      SceneLinearColor t_color = interpolate_fresnel_color(-wr, wm, eta, F0, cspec0);
