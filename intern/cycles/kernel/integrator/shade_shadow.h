...
ccl_device_inline SceneLinearColor integrate_transparent_surface_shadow(KernelGlobals kg,
...
#  ifdef __VOLUME__
      if (!integrator_state_shadow_volume_stack_is_empty(kg, state)) {
        SceneLinearColor throughput = INTEGRATOR_STATE(state, shadow_path, throughput);
...
#  endif
    ...
    if (hit < num_recorded_hits) {
      const SceneLinearColor shadow = integrate_transparent_surface_shadow(kg, state, hit);
      const SceneLinearColor throughput = INTEGRATOR_STATE(state, shadow_path, throughput) * shadow;
      if (is_zero(throughput)) {
        return true;
      }
