...
ccl_device_inline SceneLinearColor subsurface_disk_eval(const SceneLinearColor radius,
                                                     float disk_r,
                                                     float r)
{
  const SceneLinearColor eval = bssrdf_eval(radius, r);
  const float pdf = bssrdf_pdf(radius, disk_r);
  return (pdf > 0.0f) ? eval / pdf : zero_scene_linear_color();
}
...

{
  ...
  SceneLinearColor weights[BSSRDF_MAX_HITS]; /* TODO: zero? */
  ...
  for (int hit = 0; hit < num_eval_hits; hit++) {
    ...
    /* Evaluate profiles. */
    const SceneLinearColor weight = subsurface_disk_eval(radius, disk_r, r) * w;
  ...
  for (int hit = 0; hit < num_eval_hits; hit++) {
    const SceneLinearColor weight = weights[hit];
