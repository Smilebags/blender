...
// TODO(chris.cook): figure out PackedSceneLinearColor
...
struct ccl_try_align(16) SceneLinearColor
{
...
ccl_device_inline float3 make_scene_linear_color(float f);
ccl_device_inline float3 make_scene_linear_color(float x, float y, float z);
ccl_device_inline void print_scene_linear_color(const char *label, const float3 &a);
